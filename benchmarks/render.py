...
def generate_simple_readme():
    ...
    content.append(f"Run at: {datetime.fromtimestamp(data['run_at']).strftime('%a %d %b %Y, %H:%M')}  ")
    ...
    for bench_type, bench_title in [('raw', 'Raw Sockets'), ('stream', 'Streams'), ('proto', 'Protocol'), ('concurrency', 'Concurrency')]:
        ...
        for msg_size, msg_label in [('1024', '1KB'), ('10240', '10KB'), ('102400', '100KB')]:
            ...
            for loop in ['veloxloop', 'asyncio', 'uvloop']:
                ...
                content.append(f"| **{loop}** | {rps} | {mean} | {p99} | {min_lat} | {max_lat} |")
    ...
    if 'concurrency' in results:
        ...
        content.append("TCP echo server performance with different concurrency levels (1KB messages).\n")
        ...
        for loop in ['veloxloop', 'asyncio', 'uvloop']:
            ...
            for c_level in concurrency_levels:
                if c_level in concurrency_data[loop] and '1024' in concurrency_data[loop][c_level]:
