...
if not (Path(SERVER_CERT).exists() and Path(SERVER_KEY).exists()):
    try:
        subprocess.check_call([
            ...
            "-days", "1", "-keyout", SERVER_KEY, "-out", SERVER_CERT,
            "-subj", "/CN=localhost"
        ])
    except Exception:
...
class TestSSLClientConnection:
    ...
    def test_https_connection_google(self):
        ...
        async def run_test():
            ...
            try:
                await asyncio.wait_for(protocol.data_received_event.wait(), timeout=10.0)
        ...
        async def run_test():
            ...
            class InfoProtocol:
                ...
                def connection_made(self, transport):
                    ...
                    self.extra_info['ssl_context'] = transport.get_extra_info('sslcontext')
