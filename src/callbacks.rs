use pyo3::prelude::*;
use pyo3::types::{PyTuple, PyString, PyInt, PyBytes};
    ...
    fn __call__(&self, py: Python<'_>) -> PyResult<()> {
        ...
        unsafe {
            ...
            if client_fd >= 0 {
                ...
                self.future.bind(py).borrow().set_result(py, result.into())?;
...
impl SockRecvCallback {
    fn __call__(&self, py: Python<'_>) -> PyResult<()> {
        ...
        unsafe {
            let n = libc::recv(self.fd, buf.as_mut_ptr() as *mut libc::c_void, self.nbytes, 0);
